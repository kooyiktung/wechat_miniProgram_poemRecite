...
Component({
  ...
  properties: {
    ...
    user: {   //navbarData   由父页面传递的数据，变量名字自命名
      type: Array,
      value: [],
      observer: function (newVal, oldVal) { this.getAllArr() }
    }
  },
    ...
    getCurrentArr() {
      ...
      if (currentMonthDateLen > 0) {
        for (let i = 1; i <= currentMonthDateLen; i++) {
          var today = String(this.data.currentYear) + "/" + String(this.data.currentMonth) + "/" + String(i)
          if (app.globalData.user.check_in.indexOf(today) >= 0) {
            var check_in = true
          }
          else {
            var check_in = false
